...
# 🔹 تحميل البيانات الأصلية المحفوظة مسبقًا
original_data = load_data(ORIGINAL_DATA_FILE)

# 🔹 حفظ نسخة من البيانات الأصلية عند أول تشغيل بعد رفع الملف
if uploaded_file and original_data is None:
    save_data(df, ORIGINAL_DATA_FILE)  # حفظ النسخة الأصلية

# 🔹 زر لاسترجاع البيانات الأصلية
if st.button("🔄 استرجاع البيانات الأصلية"):
    df = load_data(ORIGINAL_DATA_FILE)  # تحميل النسخة الأصلية
